...
/*   Updated: 2023/03/05 16:49:46 by jthomas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_print(int fd, const char format, va_list args, int *error)
{
	...
	if (format == 'u')
		return (ft_putunsigned_fd(va_arg(args, long long int), fd, error));
	...
	if (format == 'p')
	{
		...
		res += ft_putunsignedhex_fd(
				va_arg(args, unsigned long long), fd, error);
	}
	return (res);
}
